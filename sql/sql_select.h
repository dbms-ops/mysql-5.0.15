...
typedef struct st_rollup {
    enum State {
        STATE_NONE, STATE_INITED, STATE_READY
    };
    State state;
...
public:
    /*
     * join_tab：用于本查询方案的 JOIN_TAB 描述符的一个数组
     * */
    JOIN_TAB *join_tab, **best_ref;
    JOIN_TAB **map2table;    // mapping between table indexes and JOIN_TABs
    JOIN_TAB *join_tab_save; // saved join_tab for subquery reexecution
    /*
     * table: 用于本链接的表的数量。同时是 join_tab 数组的基数
     * */
    TABLE **table, **all_tables, *sort_by_table;
    /*
     * const_tables：常规表的数量，同时是 join_tab数组的基数
     * */
    uint tables, const_tables;
    uint send_group_parts;
    bool sort_and_group, first_record, full_join, group, no_field_update;
    /*
     * 表示是否将 SELECT 的结果发送个客户端
     * */
    bool do_send_rows;
    /*
    ...
    */
    ...
    /*
     * const_table_map:显示哪个表是常数的位掩码
     * */
    table_map const_table_map, found_const_table_map, outer_join;
    /*
     * examined_rows：迄今为止所检查的记录的总数量
     * */
    ...
    */
    ha_rows fetch_limit;
    /*
     * positions：表位置的临时存储数组，用于计算最佳连接顺序
     * best_positions：在计算最佳连接顺序期间追踪目前已知的最佳连接顺序
     * */
    POSITION positions[MAX_TABLES + 1], best_positions[MAX_TABLES + 1];
    /*
     * best_read：best_positions 中定义的连接操作的相对成本度量
     * */
    double best_read;
    /*
     * fields：SELECT 语句中的列列表
     * */
    List<Item> *fields;
    ...
    /*
     * unit：指向当前选择 lex 单元
     * 一个选择 lex 单元或者是一个 SELECT，或者是一个 SELECT 语句的并集
     * */
